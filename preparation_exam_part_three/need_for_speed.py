def store_cars_info(cars_dictionary, number):
...
def special_commands(cars_dictionary):
    while True:
        ...
        if current_command == "Drive":
            ...
            fuel = int(command[2])
            available_fuel = int(cars_dictionary[brand][1])
            ...
            current_mileage = int(cars_dictionary[brand][0])
...
def print_func(cars_dictionary):
...
def need_for_speed(number):
    ...
    store_cars_info(cars_dictionary, number)
    special_commands(cars_dictionary)
    print_func(cars_dictionary)


number_iter = int(input())
need_for_speed(number_iter)
